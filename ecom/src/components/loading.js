...
const ConcentricSquaresLoader = ({ isVisible }) => {
  return (
    <div
      className={`fixed inset-0 z-50 transition-opacity duration-700 ease-in-out ${isVisible ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
    >
      <div className="min-h-screen bg-black relative overflow-hidden flex items-center justify-center">
        ...
        <div
          className="absolute top-16 right-12 w-3 h-3 border border-yellow-400 opacity-40"
          style={{ animation: "spin 8s linear infinite" }}
        ></div>
        <div
          className="absolute bottom-20 left-16 w-5 h-5 border border-gray-500 opacity-25"
        ...
        <div className="relative w-32 h-32">
          <div
            className="absolute inset-0 border-2 border-gray-600 animate-spin rounded-sm"
            style={{
          ...
          ></div>
          <div
            className="absolute inset-2 border-2 border-gray-500 rounded-sm"
            style={{
              animation: "spin 2.2s linear infinite reverse",
            }}
          ></div>
          <div
            className="absolute inset-4 border-2 border-gray-400 animate-spin rounded-sm"
            style={{
          ...
          ></div>
          <div
            className="absolute inset-6 border-2 border-gray-300 rounded-sm"
            style={{
              animation: "spin 1.4s linear infinite reverse",
            }}
          ></div>
          <div
            className="absolute inset-8 border-2 border-yellow-400 animate-spin rounded-sm"
            style={{
              animationDuration: "1s",
              animationTimingFunction: "linear",
            }}
          ></div>
          <div className="absolute top-1/2 left-1/2 w-2 h-2 bg-yellow-400 rounded-full transform -translate-x-1/2 -translate-y-1/2 animate-pulse"></div>
        </div>
